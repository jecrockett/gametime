...
function FoodGenerator(width, height) {
  ...
  this.coorGenerator = new OpenCoordinates(width, height);
}
...
FoodGenerator.prototype = {
  seedFood: function(players) {
    var food = [];
    for(var i = 0; i < 200; i++) {
      var foodItem = new Food(this.coorGenerator.create(players));
      food.push(foodItem);
    }
    return food;
  },
  ...
  seedSpeedBoosts: function(players) {
    var speedBoosts = [];
    for(var i = 0; i < 15; i++) {
      var boost = new SpeedBoost(this.coorGenerator.create(players));
  ...
  seedViruses: function(players) {
    var viruses = [];
    for(var i = 0; i < 8; i++) {
      viruses.push(new Virus(this.coorGenerator.create(players)));
    }
    return viruses;
  },
  ...
  replaceFood: function(allFood, speedBoosts, players, viruses) {
    ...
    while(speedBoosts.length < 15) {
      speedBoosts.push(new SpeedBoost(this.coorGenerator.create(players)));
    }
